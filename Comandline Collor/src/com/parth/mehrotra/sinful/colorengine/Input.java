package com.parth.mehrotra.sinful.colorengine;
...
public class Input {
	...
	public static void main(String args[]) {
		...
		System.out.println("Enter your G value");
		int g = input.nextInt();

		System.out.println("Enter your B value");
		int b = input.nextInt();

		System.out.println("INPUT RGB: " + r + ", " + g + ", " + b);

		int difference[][] = new int[180][3];

		for (int i = 0; i < 180; i++) {
			difference[i][0] = (int) (Math.pow(r - COLORS[i][0], 2));
			difference[i][1] = (int) (Math.pow(g - COLORS[i][1], 2));
			difference[i][2] = (int) (Math.pow(b - COLORS[i][0], 2));
		}

		int distance[] = new int[180];
		for (int i = 0; i < 180; i++) {
			distance[i] = (int) (Math.sqrt((difference[i][0])
					+ (difference[i][1]) + (difference[i][2])));
		}

		for (int i = 0; i < 180; i++) {
			System.out.println("SHADE: " + i + " DISTANCE: " + distance[i]);
		}
		int[] sortPos = new int[180];

		for (int i = 0; i < sortPos.length; i++) {
			sortPos[i] = i;
		}

		for (int _g = 0; _g < distance.length - 1; _g++) {
			for (int i = 1; i < distance.length - _g; i++) {
				if (distance[i - 1] > distance[i]) {
					int temp = distance[i - 1];
					distance[i - 1] = distance[i];
					distance[i] = temp;
					int temp2 = sortPos[i - 1];
		...
		for (int i = 0; i < 180; i++) {
			System.out.println("SHADE: " + sortPos[i] + " DISTANCE: "
					+ distance[i]);
		}
		...
		// break;
		//
		// if (((r > (COLORS[i][0] - allowance)) && (r < (COLORS[i][0] +
		// allowance))) && ((g > (COLORS[i][1] - allowance)) && (g <
		// (COLORS[i][1] + allowance))) && (b > (COLORS[i][2] - allowance)) &&
		// (b < (COLORS[i][2] + allowance))) {
		// matches[matchPlace] = i;
		// matchPlace++;
		// }
		// i++;
		//
		...
		// System.out.println(NAMES[matches[a]]+": "+COLORS[matches[a]][0]+", "+COLORS[matches[a]][1]+", "+COLORS[matches[a]][2]+"["+matches[a]+"]");
		// }
